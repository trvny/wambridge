...
def choose_start_volume(
    ...
    if current_volume == 0:
        # Startup mutes the speaker and restores it afterwards, so a helper
        # killed in between leaves it at 0. Following that reading would start
        # the next stream silent while every other signal reports playing -
        # the exact failure this project has already lost days to. Nobody asks
        # for playback in order to hear nothing.
